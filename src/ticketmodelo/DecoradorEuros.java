...
public class DecoradorEuros extends DecoradorTicket {

    public DecoradorEuros(Modelo modelo) {
    ...
    public void conversion_divisa() {
        ...
        double resultado = convertidor.convertir(modelo.getDenominacion() * modelo.getFichas(), modelo.getMoneda_divisa(), "EUR");
        DecimalFormat formato = new DecimalFormat("0.00");
        System.out.println("Resultado de la conversión a Euros: " + resultado);

        setPremio(formato.format(resultado).replace(",", "."));
        setPremio_letras((modelo.getFormateador().format(resultado)).toUpperCase());
        setMoneda("Euros"); // Actualiza la moneda a Euros.
